...
```
GET/POST /ticket 均签（根 404=隐身门；method 分支已撤）
...
5. **验证**：本机 `curl http://localhost:12346/ticket` → 三字段 JSON；外网 `curl https://ticket.hotify.love/` 同
...
- **白名单：要做再议**（2026-08-25 裁定：默认开放是产品形态，治理走速率维度非身份准入；缝已留好——见 [whitelist.md](whitelist.md)，将来 ~50 行插入零重构）。TICKET_AUTH_TOKEN 仅作调用方配置错检测。
